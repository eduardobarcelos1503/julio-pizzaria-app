...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<CartCtx>(() => {
    ...
    const checkoutUrl: CartCtx["checkoutUrl"] = (opts) => {
      ...
      if (fee !== null) lines.push(`Taxa de entrega: ${formatBRL(fee)}`);
      lines.push(`*TOTAL: ${formatBRL(total + (fee ?? 0))}*`, "");
      lines.push(`Endereço para entrega: ${opts?.address?.trim() ?? ""}`);
